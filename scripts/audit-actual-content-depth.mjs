...
fs.writeFileSync(path.join(reportDir, 'actual-content-depth-inventory.md'), `# Structural content-quality signals\n\nThis report measures prose and editorial structure only. It is not proof of factual correctness or mechanism coverage; those are enforced by source provenance and the source-mechanism catalog.\n\n${Object.entries(summary).map(([k,v]) => `- **${k}:** ${v}`).join('\n')}\n\n| Page | Class | Words | Logic | Example | Diagram | Limits |\n| --- | --- | ---: | --- | --- | --- | --- |\n${rows.map((r) => `| ${r.file} | ${r.classification} | ${r.words} | ${r.decisionLogic ? 'yes' : 'no'} | ${r.workedExample ? 'yes' : 'no'} | ${r.diagram ? 'yes' : 'no'} | ${r.limitations ? 'yes' : 'no'} |`).join('\n')}\n`)
const unacceptable = rows.filter((row) => row.classification === 'thin' || row.classification === 'generic')
if (unacceptable.length) { console.error(unacceptable.map((row) => `${row.file}: ${row.classification} (${row.words} words)`).join('\n')); process.exit(1) }
console.log(`Structural content-quality gate OK: ${JSON.stringify(summary)}. This result does not claim factual completeness.`)
