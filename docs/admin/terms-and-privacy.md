...
```mermaid
flowchart TD
  A[User accepts terms] --> B[Uses scoped platform features]
  B --> C[Uploads and activity are handled under platform rules]
  C --> D[Admin monitoring and audit safeguards]
  D --> E[Privacy/support request to platform administrator]
```
